...
import { serialize, deserialize } from '../../../src/core/project';
import { createBuffer, setPixel } from '../../../src/core/buffer';
import type { Project, Frame, Layer, RGBA } from '../../../src/core/types';

const C: RGBA = [255, 106, 26, 255];

const mkLayer = (id: string, w: number, h: number, paint = false): Layer => ({
  id, name: id, visible: true, locked: false, opacity: 100, blend: 'normal',
...
const mkProject = (): Project => {
  ...
  return {
    schema: 1, id: 'p1', name: 'Test', w, h, frames, palette: null,
    indexed: false, fps: 12, createdAt: '2026-07-01T00:00:00Z',
...
describe('project round-trip', () => {
  it('serialize -> deserialize is lossless for a multi-layer, multi-frame project', () => {
    ...
    if (!r.ok) return;
    const q = r.value;
    expect(q.w).toBe(p.w); expect(q.h).toBe(p.h);
    ...
    expect(Array.from(q.frames[0].layers[0].buffer.data))
      .toEqual(Array.from(p.frames[0].layers[0].buffer.data));
    expect(Array.from(q.frames[1].layers[1].buffer.data))
      .toEqual(Array.from(p.frames[1].layers[1].buffer.data));
  });
